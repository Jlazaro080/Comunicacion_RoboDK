...
try:
  ...
  from robodk.robomath import pause, eye, Mat
...
# ===================== PIEZA QSK95 =====================
pieza_QSK95 = RDK.Item('QSK95', ITEM_TYPE_OBJECT)
if not pieza_QSK95.Valid():
  print("ADVERTENCIA: No se encontro la pieza 'QSK95' en la escena. El Attach/Detach no tendra efecto.")
  pieza_QSK95 = None

# Guardar la rotación original de la pieza
_pieza_rot_original = None
if pieza_QSK95 and pieza_QSK95.Valid():
  _pieza_rot_original = pieza_QSK95.PoseAbs()


def _get_robot_tool(robot):
  """Obtiene la herramienta activa del robot. Fallback al robot si no hay tool."""
  try:
...
def attach_pieza(pieza, robot):
  """Pick: adjunta al TCP manteniendo posición/orientación mundial actual. Sin girar."""
  if pieza is None or not pieza.Valid():
    return
  parent = _get_robot_tool(robot)
  piece_world = pieza.PoseAbs()  # Pose actual (posición + orientación)
  parent_world = parent.PoseAbs()  # Pose del TCP
  pieza.setParent(parent)  # Reparenta al TCP
  # Calcular pose local que preserva la pos/rot mundial
  local_pose = parent_world.inv() * piece_world
  pieza.setPose(local_pose)
  RDK.Render()
  print(f"Pieza adjuntada a: {parent.Name()}")


def detach_pieza(pieza, frame):
  """Place: suelta la pieza en el origen del frame (sin offset)."""
  if pieza is None or not pieza.Valid():
    return
  pieza.setParent(frame)    # Reparenta al frame de la estación
  pieza.setPose(eye(4))     # Deja la pieza EXACTAMENTE en el origen del frame (sin offset)
  RDK.Render()
  print(f"Pieza soltada en: {frame.Name()}")
...
def set_station_indicator(op_key, ocupada: bool, robot_name=None):
  ...
  Busca primero formato por robot: Ind_R1_OP_20_Libre/Ocupada
  ...
  if robot_name:
    candidate_pairs.append((
      ...
      f"Ind_{robot_name}_{op_key}_Ocupada",
    ))
  candidate_pairs.append((
    f"Ind_{op_key}_Libre",
    f"Ind_{op_key}_Ocupada",
  ))

  for name_libre, name_ocupada in candidate_pairs:
    ind_libre = RDK.Item(name_libre)
    ind_ocupada = RDK.Item(name_ocupada)

    if not ind_libre.Valid() and not ind_ocupada.Valid():
      continue
...
def initialize_station_indicators(stations):
  """Inicializa en estado Libre las estaciones indicadas (si existen indicadores)."""
  for robot_name, op_key in stations:
    set_station_indicator(op_key, ocupada=False, robot_name=robot_name)
...
def execute_operation_cycle(robot, robot_name, frame, pos_afuera, pos_dentro, pos_x, speed_table, time_table, op_key, piece_fn=None):
  COLOR_ACTIVE = [1.0, 1.0, 0.0, 1.0]  # amarillo: estacion activa
  COLOR_OK     = [0.0, 0.8, 0.0, 1.0]  # verde: completada OK
  COLOR_WARN   = [1.0, 0.2, 0.0, 1.0]  # rojo: advertencia/error
  ...
  try:
    ...
    frame.Recolor(COLOR_ACTIVE)
    RDK.Render()
    set_station_indicator(op_key, ocupada=True, robot_name=robot_name)
    ...
    # Attach o Detach de pieza al llegar a pos_x (Pick o Place)
    if piece_fn is not None:
      try:
        piece_fn()
      except Exception as pfn_exc:
        print(f"ADVERTENCIA: piece_fn en {op_key} fallo: {format_exception(pfn_exc)}")
    ...
    frame.Recolor(COLOR_OK)
    RDK.Render()
    set_station_indicator(op_key, ocupada=False, robot_name=robot_name)
    return True
  except Exception as exc:
    frame.Recolor(COLOR_WARN)
    RDK.Render()
    set_station_indicator(op_key, ocupada=False, robot_name=robot_name)
...
def run_timed_operation(op_name, operation_fn):
  ...
  RDK.ShowMessage(f"Ejecutando: {op_name}", False)
  op_start = timer()
  ok = operation_fn()
  elapsed = timer() - op_start
  status = "OK" if ok else "WARN"
  finish_msg = f"Fin {op_name} [{status}] | {elapsed:.3f} s"
  print(finish_msg)
  RDK.ShowMessage(finish_msg, False)
...
#Posiciones de  Robot 2 operacion 150
RDK_R2_Op_150_Frame = get_required_item('R2_Op_150', ITEM_TYPE_FRAME)
RDK_R2_Op_150_Pos_Dentro = get_required_item('R2_Op_150_Pos_Dentro')
RDK_R2_Op_150_Pos_X = get_required_item('R2_Op_150_Pos_X')
RDK_R2_Op_150_Pos_Afuera = get_required_item('R2_Op_150_Pos_Afuera')


#Posiciones de Robot 2 operacion 160  
RDK_R2_Op_160_Frame = get_required_item('R2_Op_160', ITEM_TYPE_FRAME)
RDK_R2_Op_160_Pos_Dentro = get_required_item('R2_Op_160_Pos_Dentro')
RDK_R2_Op_160_Pos_X = get_required_item('R2_Op_160_Pos_X')
RDK_R2_Op_160_Pos_Afuera = get_required_item('R2_Op_160_Pos_Afuera')
...
# Estaciones a monitorear visualmente (Libre/Ocupada)
initialize_station_indicators([
  ("R1", "OP_00"),
  ("R1", "OP_10"),
  ("R1", "OP_20"),
  ("R1", "OP_30_A"),
...
def R2_Op_150(piece_fn=None):
  run_timed_operation(
    ...
    lambda: execute_operation_cycle(
      ...
      "R2",
      RDK_R2_Op_150_Frame,
      RDK_R2_Op_150_Pos_Afuera,
      RDK_R2_Op_150_Pos_Dentro,
      RDK_R2_Op_150_Pos_X,
      R2_OPERATION_SPEEDS,
      R2_OPERATION_TIMES_SEC,
      "OP_150",
...
def R2_Op_160(piece_fn=None):
  run_timed_operation(
    ...
    lambda: execute_operation_cycle(
      ...
      "R2",
      RDK_R2_Op_160_Frame,
      RDK_R2_Op_160_Pos_Afuera,
      RDK_R2_Op_160_Pos_Dentro,
      RDK_R2_Op_160_Pos_X,
      R2_OPERATION_SPEEDS,
      R2_OPERATION_TIMES_SEC,
      "OP_160",
...
def R1_pick_and_place_op_00_to_10():
  R1_Op_00(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))          # Pick en OP_00
  R1_Op_10(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_10_Frame))  # Place en OP_10


def R1_pick_and_place_op_10_to_20():
  R1_Op_10(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))          # Pick en OP_10
  R1_Op_20(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_20_Frame))  # Place en OP_20

def R1_pick_and_place_op_20_to_30_A():
  R1_Op_20(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))             # Pick en OP_20
  R1_Op_30_A(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_30_A_Frame))  # Place en OP_30_A

def R1_pick_and_place_op_20_to_30_B():
  R1_Op_20(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))             # Pick en OP_20
  R1_Op_30_B(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_30_B_Frame))  # Place en OP_30_B

def R1_pick_and_place_op_30_A_to_60():
  R1_Op_30_A(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))          # Pick en OP_30_A
  R1_Op_60(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_60_Frame))  # Place en OP_60

def R1_pick_and_place_op_30_B_to_60():
  R1_Op_30_B(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))          # Pick en OP_30_B
  R1_Op_60(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_60_Frame))  # Place en OP_60

def R1_pick_and_place_op_60_to_70():
  R1_Op_60(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR1))             # Pick en OP_60
  R1_Op_70(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R1_Op_70_Frame))  # Place en OP_70 (transferencia a R2)

#Funciones para Robot 2



def R2_pick_and_place_op_70_to_90():
  R2_Op_70(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))          # Pick en OP_70 (toma de R1)
  R2_Op_90(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_90_Frame))  # Place en OP_90

def R2_pick_and_place_op_90_to_100():
  R2_Op_90(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_90
  R2_Op_100(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_100_Frame))  # Place en OP_100

def R2_pick_and_place_op_100_to_110():
  R2_Op_100(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_100
  R2_Op_110(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_110_Frame))  # Place en OP_110


def R2_pick_and_place_op_110_to_120():
  R2_Op_110(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_110
  R2_Op_120(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_120_Frame))  # Place en OP_120

def R2_pick_and_place_op_120_to_130():
  R2_Op_120(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_120
  R2_Op_130(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_130_Frame))  # Place en OP_130

def R2_pick_and_place_op_130_to_140():
  R2_Op_130(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_130
  R2_Op_140(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_140_Frame))  # Place en OP_140

def R2_pick_and_place_op_140_to_150():
  R2_Op_140(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_140
  R2_Op_150(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_150_Frame))  # Place en OP_150

def R2_pick_and_place_op_150_to_160():
  R2_Op_150(piece_fn=lambda: attach_pieza(pieza_QSK95, robotR2))           # Pick en OP_150
  R2_Op_160(piece_fn=lambda: detach_pieza(pieza_QSK95, RDK_R2_Op_160_Frame))  # Place final en OP_160
...
print("Ejecutando pick and place OP 70 to OP 90")
R2_pick_and_place_op_70_to_90()
...
print("Ejecutando pick and place OP 140 to OP 150")
R2_pick_and_place_op_140_to_150()

print("Ejecutando pick and place OP 150 to OP 160")
R2_pick_and_place_op_150_to_160()
...
cycle_msg = f"Tiempo ciclo: {cycle_time:.3f} s"
print(cycle_msg)
RDK.ShowMessage(cycle_msg, False)